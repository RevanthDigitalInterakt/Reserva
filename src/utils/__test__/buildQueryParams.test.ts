...
describe('buildQueryParams', () => {
  ...
  it('should encode non-ASCII characters correctly', () => {
    ...
    const result = buildQueryParams(params);
    expect(result).toBe('name=Jhn&city=Mnchen');
  });
  ...
  it('should handle nested objects correctly', () => {
    ...
    const result = buildQueryParams(params);
    expect(result).toBe('details-address=123_Main_St&details-city=Anytown');
  });

  it('should handle nested objects correctly case 2', () => {
    const params = { details: { address: { street: 'Main St', number: 123 }, city: 'Anytown' } };
    const result = buildQueryParams(params);
    expect(result).toBe('details-address-street=Main_St&details-address-number=123&details-city=Anytown');
  });
});
