...
const MenuSubItem: React.FC<IMenuSubItem> = ({ title, onPress, highlight }) => {
	return (
		<Box
			...
			paddingX="xxs"
		>
			<Typography fontSize={13} fontFamily="nunitoRegular" fontWeight={highlight ? "bold" : "normal"}>
				{ title }
			</Typography>
		</Box>
	);
...
const MenuItem: React.FC<IMenuItem> = ({ title, opened, onPress, index, subItemList, highlight }) => {
	return (
		<Box>
      		<TouchableOpacity onPress={() => onPress(index)}>
			<Box justifyContent="space-between" marginY="micro" flexDirection="row" marginX="xxs">
		
					<Typography color={highlight ? "vermelhoAlerta" :  "preto" } fontSize={13} fontFamily="nunitoBold" fontWeight="bold">
						{title.toUpperCase()}
					</Typography>
        ...
        <Divider variant="fullWidth" marginTop="micro" />
				<Animatable.View animation="fadeIn">
					{subItemList.map((item) => {
						return <MenuSubItem highlight={item.highlight} title={item.title} onPress={() => {}} />;
					})}
				</Animatable.View>
        </>
			)}
