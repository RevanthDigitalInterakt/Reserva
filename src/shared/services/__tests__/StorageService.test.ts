...
describe('StorageService', () => {
  describe('method: setItem()', () => {
    it.skip('should resolve a promise with no errors and return true when save data', async () => expect(
  ...
  describe('method: getItem()', () => {
    it.skip('should resolve a promise and return saved data', async () => expect(
      StorageService.getItem({ key: StorageServiceKeys.INSTALLATION_TOKEN }),
    ).resolves.toEqual(mockReturnValues.installationToken));

    it.skip('should return with no errors when key is found', async () => StorageService.getItem({
      key: StorageServiceKeys.INSTALLATION_TOKEN,
    }).catch((error) => {
      expect(error).toBeNull();
    }));

    it.skip('should reject and return Error() when if key is not found', async () => expect(
